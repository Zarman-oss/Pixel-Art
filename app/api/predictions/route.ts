...
export async function POST(request: Request) {
  try {
    ...
    // Check if jsonStartResponse contains the 'urls' and 'get' properties
    if (!jsonStartResponse.urls || !jsonStartResponse.urls.get) {
      console.error('Invalid API response structure:', jsonStartResponse);
      return new Response('Failed to initiate image generation', {
        status: 500,
      });
    }
